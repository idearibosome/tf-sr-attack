...
tf.flags.DEFINE_string('data_input_path', 'LR', 'Base path of the input images.')
...
class BasicLoader(BaseLoader):
  ...
  def prepare(self):
    ...
    input_path = FLAGS.data_input_path
    self.image_name_list = [f for f in os.listdir(input_path) if (f.lower().endswith('.png') or f.lower().endswith('.jpg'))]
    ...
    with self.tf_image_graph.as_default():
      ...
      tf_image = tf.image.decode_image(tf_image, channels=FLAGS.data_channels, dtype=tf.uint8)
  ...
  def get_patch_batch(self, batch_size, input_patch_size):
    input_list = []
    truth_list = []

    batch_size = min(batch_size, self.get_num_images())

    for image_index in range(batch_size):
      input_image, truth_image, _ = self.get_image_pair(image_index)

      if (input_patch_size > 0):
        scale = FLAGS.scale
  ...
  def get_random_image_patch_pair(self, input_patch_size):
    # select an image
    image_index = np.random.randint(self.get_num_images())

    # retrieve image
    input_patch, truth_patch = self.get_image_patch_pair(image_index=image_index, input_patch_size=input_patch_size)
    
    # finalize
    return input_patch, truth_patch


  def get_image_patch_pair(self, image_index, input_patch_size):
    # retrieve image
    input_image, truth_image, _ = self.get_image_pair(image_index=image_index)
  ...
  def get_image_pair(self, image_index):
    # retrieve image
    image_name = self.image_name_list[image_index]
    input_image = self._get_input_image(image_name)
  ...
  def _get_input_image(self, image_name):
    image = None
    has_cached = False
    ...
    if (image is None):
      image_path = os.path.join(FLAGS.data_input_path, image_name)
